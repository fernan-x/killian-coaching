import { Home, Target, Users, Truck, CheckCircle, Building2, Clock, Heart, Briefcase, Zap, Video, Wifi, Calendar } from "lucide-react";
...
type Tab = 'domicile' | 'entreprise' | 'visio';
const TABS: Tab[] = ['domicile', 'entreprise', 'visio'];

const CoachingSection = () => {
  const [activeTab, setActiveTab] = useState<Tab>('domicile');
  const tabRefs = {
    domicile: useRef<HTMLButtonElement>(null),
    entreprise: useRef<HTMLButtonElement>(null),
    visio: useRef<HTMLButtonElement>(null),
  };

  const handleKeyDown = (e: React.KeyboardEvent, tab: Tab) => {
    const currentIndex = TABS.indexOf(tab);
    let nextTab: Tab | null = null;
    if (e.key === 'ArrowRight') nextTab = TABS[(currentIndex + 1) % TABS.length];
    if (e.key === 'ArrowLeft') nextTab = TABS[(currentIndex - 1 + TABS.length) % TABS.length];
    if (nextTab) {
  ...
  const featuresHome = [
    { icon: Home, title: "Chez vous", description: "Entraînez-vous dans le confort de votre domicile, sans déplacement ni perte de temps." },
    { icon: Truck, title: "Matériel fourni", description: "Je me déplace avec tout l'équipement nécessaire : haltères, élastiques, tapis, et plus." },
    { icon: Target, title: "Sur-mesure", description: "Programme 100% adapté à vos objectifs, votre niveau et vos contraintes." },
    { icon: Users, title: "Suivi personnalisé", description: "Un accompagnement régulier pour vous motiver et mesurer vos progrès." },
  ];

  const featuresEntreprise = [
    { icon: Building2, title: "Sur site", description: "Je me déplace directement dans vos locaux pour des séances pratiques et accessibles." },
    { icon: Users, title: "Cours collectifs", description: "Séances en groupe pour renforcer la cohésion d'équipe et la motivation collective." },
    { icon: Clock, title: "Horaires flexibles", description: "Séances adaptées à vos plannings : pause déjeuner, avant/après le travail." },
    { icon: Heart, title: "Bien-être au travail", description: "Réduisez le stress et améliorez la qualité de vie de vos collaborateurs." },
  ];

  const featuresVisio = [
    { icon: Video, title: "En visio", description: "Séances en direct par visioconférence, où que vous soyez en France ou à l'étranger." },
    { icon: Wifi, title: "Sans déplacement", description: "Une simple connexion internet et un peu d'espace suffisent pour vous entraîner." },
    { icon: Target, title: "Programme adapté", description: "Exercices choisis en fonction du matériel dont vous disposez chez vous." },
    { icon: Calendar, title: "Horaires flexibles", description: "Plus de contraintes géographiques : on cale les séances quand ça vous arrange." },
  ];
  ...
  const tabConfig = {
    domicile: { label: 'Coaching à domicile', icon: Home },
    entreprise: { label: 'Coaching en entreprise', icon: Briefcase },
    visio: { label: 'Coaching en visio', icon: Video },
  };

  return (
    <section id="coaching" className="section-padding bg-background">
      <div className="container-custom">
        {/* Tab Selector */}
        <div className="flex justify-center mb-12">
          <div
            role="tablist"
            aria-label="Type de coaching"
            className="inline-flex flex-wrap justify-center bg-secondary rounded-2xl p-2 gap-2"
          >
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                    activeTab === tab
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {features.map((feature, index) => (
              <div
                key={feature.title}
                className="bg-card rounded-2xl p-6 shadow-custom-sm hover:shadow-custom-md transition-all duration-300 card-hover group"
                  ...
                  {activeTab === 'domicile' ? (
                    <>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-primary text-primary-foreground text-sm flex items-center justify-center flex-shrink-0" aria-hidden="true">1</span>
                        <span>Séance découverte gratuite pour définir vos objectifs</span>
                      </li>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-primary text-primary-foreground text-sm flex items-center justify-center flex-shrink-0" aria-hidden="true">2</span>
                        <span>Programme personnalisé créé sur-mesure</span>
                      </li>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-primary text-primary-foreground text-sm flex items-center justify-center flex-shrink-0" aria-hidden="true">3</span>
